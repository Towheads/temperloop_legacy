...
| **Dual-build ledger row schema** | anything reading `.temperloop/model-comparison/dual-build/rows.jsonl`'s per-item, per-arm row shape (`schema_version`, `seq`, `tier`/`model`/`arm`, `base_sha`/`head_sha`, `gate`, `cost`, `judge`, `pick`, `override`, `loss_reason`, `cross_read_attempted`, `guard_armed`) or its sibling `calibration-pairs.jsonl`/`calibration.json` shape — the report producer, a future cross-repo rollup, or a manual analysis; a field addition is additive, a field removal or type change is breaking (ADR 0039) | `workflows/scripts/model-comparison/dual-build-ledger.sh` (the schema's own documented header), `workflows/scripts/report-producers/dual-build` |
