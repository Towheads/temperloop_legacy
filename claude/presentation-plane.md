...
| Dual-build ledger row shape (`schema_version, seq, created_at, tier, model, slug, arm, base_sha, head_sha, start_order, gate, cost{…}, judge{…}, pick, override, loss_reason, cross_read_attempted, guard_armed, machinery_version, operator, host, repo`) in `.temperloop/model-comparison/dual-build/rows.jsonl`, plus the `archives/<slug>@<arm>.patch` filename convention | **Frozen** | `workflows/scripts/model-comparison/dual-build-ledger.sh` header comment (the one place the shape is authored) | temperloop#2072, epic #2065 "new-work dual-build harness". `read`'s own consistency check keys on the exact `seq`/`slug`/`arm` field names to detect a truncated ledger, and `archive-check` derives a missing `--base` from the same-named `slug`/`arm`/`base_sha` fields; `append` assigns `schema_version`/`seq` itself and ignores any caller-supplied value for either, so a restyled field name would desync the reader from data the writer already committed to disk (a single-host, append-only, git-ignored stream — never git-tracked, so there is no diff to catch a silent rename after the fact). `repo` (temperloop#2119) is the one OPTIONAL field: rows written before it existed carry no such key, so every reader treats it as absent-able (`.repo // null`) rather than required — adding it was additive per VERSIONING.md's own row, removing it or making it mandatory would not be. |
| Dual-build A/A noise-floor record shape (`schema_version, n, judge_model, arm_model, margins[], order_agreement_rate, arm_failure_rate, recorded_at`) in `.temperloop/model-comparison/dual-build/null-floor.json`, plus `null-floor-status`'s in-band `outcome` key (`NULL_FLOOR` \| `NULL_FLOOR_UNAVAILABLE` + `reason`) | **Frozen** | `workflows/scripts/model-comparison/dual-build-ledger.sh` header comment, its NULL-FLOOR MODE section (the one place the shape is authored) | temperloop#2271 (epic #2237, split from #2209). `null-floor-record` writes the file and `null-floor-status` reads it back by these exact field names — the reader refuses (`NULL_FLOOR_UNAVAILABLE`, exit non-zero) a file missing any of them or carrying a `schema_version` it does not know, so a restyled key desyncs the reader from data the writer already committed to a git-ignored, untracked file with no diff to catch it. The two `outcome` literals are what a later level-pick/report consumer branches on (they join `build-level.mjs`'s closed outcome set in a later item), so a reworded outcome reads there as an unknown verdict, not a friendlier label. |
...
| Escalation record's `claim_disposition` (`hold` \| `release`) + `resumable_state.{retained,evidence[].fact}` | **Frozen** | `claude/workflows/build-level.mjs` (`escalationResumableState`/`stampClaimDisposition`, the emitter) + `claude/commands/build.md` 3d-esc step 1 (the orchestrator that acts on it) | temperloop#2230. The board claim is the cross-session lock, and this field is what the orchestrator branches on to decide whether an escalated item stays In Progress or parks back to `Ready` — so the two literals are read as literals, and the `evidence[].fact` names (`worktrees_intact`, `worktree_left_intact`, `committed_work:<outcome>`) say which payload reading produced the verdict. A renamed literal silently reverts every escalation to the release path, which is the #2230 failure itself: the board reading `Ready` while worktrees hold unlanded committed work. |
